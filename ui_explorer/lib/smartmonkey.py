...
class SmartMonkey(object):
    def __init__(self, driver, package=None):
        self.driver = driver
        ...
        self.DFS_DEPTH_LIMIT = 5

    def skip_irrelevant(self, initial=True, limit=None, detect_login=True):
        ...
        try:
            ...
            if self.is_in_dialog() \
                ...
                source_before_tap = self.driver.page_source
                to_tap = safe_clickable[-1]
                ele_info = layout.ElementInfo(to_tap)
                logger.debug(u'Tapped {0}'.format(ele_info))
                self.tap(to_tap)
                if self.driver.current_activity == cur_act \
                        and difflib.SequenceMatcher(None, self.driver.page_source.lower(),
    ...
    def find_elements_by_keyword(self, keyword, clickable_only=False, exact=False, scroll=False,
        ...
        if text_max_len or sort_elements:
            ...
            if sort_elements:
                len_elements.sort()
            elements = [e for (_, e) in len_elements]
            if elements:
    ...
    def tap_test(self, element=None, x=None, y=None, diff=0.98, timeout=3):
        if self.__class__.__name__ == 'Navigator':
            diff = 1
        act_before = self.driver.current_activity
        source_before = self.driver.page_source.lower()

        self.action.tap(element, x, y).perform()

        while True:
            diff_ratio = difflib.SequenceMatcher(None, self.driver.page_source.lower(), source_before).ratio()
            act_after = self.driver.current_activity
            if act_after != act_before or diff_ratio < diff:
                return True
            # polling
            if timeout > 0:
                sleep(1)
    ...
    def find_clickable_siblings(self, e):
        ...
        if info.bounds:
            bds = info.bounds
            bounds_str = "[%d,%d][%d,%d]" % (bds[0][0], bds[0][1], bds[1][0], bds[1][1])
            # common.debug(u'\t\tFind sibling by bounds: {}'.format(info.bounds))
            query = u"//*[@bounds='{0}']/following-sibling::*[@clickable='true']" \
                    u"| //*[@bounds='{0}']/preceding-sibling::*[@clickable='true']".format(bounds_str)
        elif info.res_id:
    ...
    def wait_for_keyword(self, keyword, timeout=10):
        t = timeout
        while t > 0:
            if re.search(keyword, self.driver.page_source, re.I):
                return True
            else:
                sleep(1)
                t -= 1
        ...
        if isinstance(destination, dict):
            ...
            while t > 0:
                for (k, v) in destination.items():
                    if re.search(v, self.driver.page_source, re.I):
                        return k
                    else:
                        sleep(1)
                        t -= 1
        ...
        if isinstance(destination, list):
            ...
            while t > 0:
                for kw in destination:
                    if re.search(kw, self.driver.page_source, re.I):
                        return True
                    else:
                        sleep(1)
                        t -= 1
...
class Explorer(SmartMonkey):
    def __init__(self, driver, package=None, scan_keywords=None, dfs_config=None,
                 dest_keywords=None, dest_activities=None, use_snapshot=None, emulator=None):
        super(Explorer, self).__init__(driver, package=package)
        ...
        if use_snapshot:
            self.set_snapshot_info(emulator, use_snapshot)
    ...
    def set_dfs_config(self, dfs_config):
        if isinstance(dfs_config, dict):
            self._dfs_config = dfs_config
        else:
            raise Exception('[Explorer] cannot set dfs: not in dict format')
    ...
    def set_snapshot_info(self, emulator, tag):
        if not (emulator and tag):
            raise Exception("Both emulator and tag name are required to use snapshot")
        self._emulator = emulator
        self._use_snapshot = tag
    ...
    def reload_snapshot(self):
        ...
        tag = self._use_snapshot
        # check emulator
        if not self._emulator:
            raise myexceptions.EmulatorActionException('No emulator instance is specified')
        # check snapshot
        tags_in_use = [x['tag'] for x in self._emulator.list_snapshot()]
        if tag not in tags_in_use:
            raise myexceptions.EmulatorActionException('No snapshot with tag {}'.format(tag))
        # try to load snapshot
        if not self._emulator.load_snapshot(tag):
            raise myexceptions.EmulatorActionException('Fail to load snapshot {}'.format(tag))
    ...
    def explore(self, result_type='attribute', algorithm='scan'):
        ...
        if algorithm == 'scan':
            ...
            while result['status'] == 'deadend':
                ...
                logger.info(u"Deadend, returning home ({}). Path: ->{}"
                            .format(cnt, u'->'.join([stop for stop in result['path']])))
                self.return_to_home()
                result = self._scan_attempt(path_kw, result_path=None, last_matched=None, result_type=result_type)
            return result
    ...
    def _dfs(self, depth):
        ...
        # Found destination
        path = [{"xpath": e.page.get_xpath(e._element.element)} for e in self.visiting_elements]
        ...
        # If is not in app, call skip_irrelevant
        not_in_app = False
        if not self.is_in_app():
            try:
                self.skip_irrelevant()
            except myexceptions.AppNotRunningException:
                not_in_app = True

        # Find next unvisited element in current page
        if not not_in_app:
            for ue in self._elements_generator(by_xml=True, score_threshold=0.11):
                status = 'untouched'
    ...
    def _elements_generator(self, by_xml=True, score_threshold=0.0, skip_irr=True):
        """
        ...
        :param by_xml: If True, parse elements from static XML.
               ...
               score_threshold: only generate element with score higher than or equal to the threshold.
               skip_irr: try to skip irrelevant if none can be generated
        :return: An element generator
        """
        layout.WEIGHTS = self._dfs_config
        if by_xml:
        ...
        # call skip_irrelevant and re-generate if none generated
        if skip_irr and len(sorted_ue) == 0:
            self.skip_irrelevant()
            for ue in self._elements_generator(by_xml=by_xml, score_threshold=score_threshold, skip_irr=False):
                yield ue
        else:
            for ue in sorted_ue:
                yield ue
...
class Navigator(SmartMonkey):
    def __init__(self, driver, package=None, path=""):
        super(Navigator, self).__init__(driver, package=package)
        self.path = {}
        if path:
    ...
    def navigate(self):
        ...
        for i in range(step):
            ...
            if 'optional' in stop and stop['optional']:
                ...
                if not re.search(stop['keyword'], self.driver.page_source, re.I):
                    if i + 1 == step \
                            or (i + 1 < step and 'xpath' in stops[i + 1]) \
                            or (i + 1 < step and re.search(stops[i + 1]['keyword'], self.driver.page_source, re.I)):
...
class Stabilizer(SmartMonkey):
    def __init__(self, driver, package=None):
        super(Stabilizer, self).__init__(driver, package=package)
