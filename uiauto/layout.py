...
class UniqueElement(object):
    ...
    def __init__(self, element):
        ...
        if element_class == 'WebElement':
            self._static = False
            self.page = UniquePage(element.parent.page_source, element.parent.current_activity)
    ...
    def _crop_bounds(im, bounds):
        ...
        box = tuple(map(int, re.findall('\d+', bounds)))
    ...
    def set_score_config(self, config=None):
        if config:
            return
        self._config['OVERALL_WEIGHTS'] = {
            'text': 0.3, 'desc': 0.2, 'res_id': 0.2, 'clickable': 0.1, 'position': 0.1, 'class': 0.1,  'hierarchy': 0.0
        }
        self._config['KEYWORD_WEIGHTS'] = [
            {'keywords': u'sina|wechat|weixin|wx|facebook|fb|微信|新浪|微博', 'weight': 1},
            {'keywords': u'login|sign|登录|登陆|登錄', 'weight': 0.9},
            {'keywords': u'我|个人|mine', 'weight': 0.8},
            {'keywords': u'user|profile|帐户|账户|账号|帐号', 'weight': 0.7},
            {'keywords': u'setting|menu|设置|菜单|drawer', 'weight': 0.6}
        ]
        self._config['CLASS_WEIGHTS'] = {
            'Button': 1,
            'ImageButton': 1,
            'TextView': 0.8,
            'ImageView': 0.7
    ...
    def score(self):
        ...
        if not self._score:
            ...
            self._score += w['clickable'] * 1 if self.info.clickable else 0
            ...
            #     w['clickable'], int(self.info.clickable)))
        return self._score
    ...
    def bounds(self):
        if self._bounds is None:
            try:
                ...
                bounds = "[%d,%d][%d,%d]" % (x, y, x + width, y + height)
            except Exception:
                bounds = ""
            self._bounds = bounds
        return self._bounds
